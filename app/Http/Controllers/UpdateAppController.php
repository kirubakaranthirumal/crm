...
class UpdateAppController extends Controller{
	 ...
	 public function show($id,Request $request){
		...
		$inputArray = $cinputArray = $appresponse = array();
		...
		if(!empty($inputArray['submit'])){
			if(
				...
				if(!empty($inputArray['editor'])){
					$cinputArray['appDesc'] = $inputArray['editor'];
				}
				else{
					$cinputArray['appDesc'] = "<br>";
				}
				...
				if(((!empty($postServiceArray)) && (!empty($responseArray->status) && ($responseArray->status=="200")))){
