...
class DeleteSettingsController extends Controller{
	...
	*/
	public function index(Request $request){
		...
		try{
			...
			$statusArray = $this->retrieveAllStatusInfo($params);
		...
		return view('admin.users.settings',['department'=>$departmentArray,'category'=>$categoryArray,'priority'=>$priorityArray,'source'=>$sourceArray,'type'=>$typeArray,'status'=>$statusArray]);
	}
	...
	*/
	...
	public function store(Request $request){
		...
		$statusCode = "200";
		$status = "Success";
		$userId = $deptId = $catId = $priorId = $sourceId = $typeId = $statusId = "";
		...
		if(!empty($inputArray['settings_type'])){
			...
			elseif($inputArray['settings_type']== "6"){
				$statusId = $this->createType($inputArray);
			}
		}
		...
		elseif(!empty($statusId)){
			session()->put('statusSuc','Status has been created successfully');
		}

		if((!empty($deptId)) || (!empty($catId)) || (!empty($priorId)) || (!empty($sourceId)) || (!empty($typeId)) || (!empty($statusId))){
	...
	*/
	public function show($id, Request $request){
		...
		$depValArray = $catValArray = $priorValArray = $sourceValArray = $typeValArray = $statusValArray = array();
		$deptUpdArrray = $catUpdArrray = $priorUpdArrray = $sourceUpdArrray = $typeUpdArrray = $statusUpdArrray = array();

		if(!empty($setId)){
			if($setId=="1"){
				$depValArray = $deptObj->deleteById($id);
			}
			elseif($setId=="2"){
				$catValArray = $categoryObj->deleteById($id);
			}
			elseif($setId=="3"){
				$priorValArray = $priorityObj->deleteById($id);
			}
			elseif($setId=="4"){
				$sourceValArray = $sourceObj->deleteById($id);
			}
			elseif($setId=="5"){
				$typeValArray = $typeObj->deleteById($id);
			}
			elseif($setId=="6"){
				$statusValArray = $statusObj->deleteById($id);
			}
		}
		...
		if(!empty($depValArray)){
			session()->put('departmentDelSuc','Department has been Deleted successfully');
		}
		elseif(!empty($catValArray)){
			session()->put('categoryDelSuc','Category has been Deleted successfully');
		}
		elseif(!empty($priorValArray)){
			session()->put('priorityDelSuc','Priority has been Deleted successfully');
		}
		elseif(!empty($sourceValArray)){
			session()->put('sourceDelSuc','Source has been Deleted successfully');
		}
		elseif(!empty($typeValArray)){
			session()->put('typeDelSuc','Type has been Deleted successfully');
		}
		elseif(!empty($statusValArray)){
			session()->put('statusDelSuc','Status has been Deleted successfully');
		}

		return view('admin.users.settings',['department'=>$departmentArray,'category'=>$categoryArray,'priority'=>$priorityArray,'source'=>$sourceArray,'type'=>$typeArray,'status'=>$statusArray,'set_id'=>$setId,'deptval'=>$depValArray,'catval'=>$catValArray,'priorityval'=>$priorValArray,'sourceval'=>$sourceValArray,'typeval'=>$typeValArray,'statusval'=>$statusValArray]);
	}
