...
class LoginController extends Controller{
	...
	public function index(){
		...
		return view('auth.login',['app_data'=>$appListArray,'input'=>$appListArray]);
	}
	...
	public function store(Request $request){
		...
		$errorArray = array();		
		if((!empty($responseArray->status)) && ($responseArray->status == 201)){			
			if(!empty($responseArray->msg)){	
				$errorArray = explode(":",$responseArray->msg);
			}
			
			if(!empty($errorArray['1'])){
				$userTypeValidationError['userType'] = $errorArray['1'];
			}
		}
		
		//print"<pre>";
		//print_r($userTypeValidationError);
		...
		if(!empty($userTypeValidationError)){
			...
			elseif((!empty($responseArray->status)) && ($responseArray->status == "201")){
				...
				return view('auth.login',['app_data'=>$appListArray,'input'=>$inputArray]);
			}
		}
	...
	public function logout(){
		...
		if((!empty($responseArray->status)) && ($responseArray->status == "200")){
			unset($_SESSION['sidebarMailArray']);
			unset($_SESSION['sidebarFolderArray']);
			Session::flush();
			return redirect('admin/login_user');
		}
	}
