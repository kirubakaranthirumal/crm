...
class TicketListController extends Controller
{
		...
		*/
		...
		if((!empty($responseArray->status)) && ($responseArray->status == "200")){
            if(!empty($responseArray->msg)){
                ...
                if(!empty($userId)){
                	return view('admin.ticket_form.tickets_list',['ticketdata' => $ticketListArray,'ticketcountdata'=> $ticketCountArray,'tab'=>$tabId,'department'=>$department,'category'=>$category,'type'=>$type,'priority'=>$priority,'priorityDisp'=>$priorityDispData,'source'=>$source,'status'=>$statusCountArray]);
                }
            }
        }
	...
	public function DeleteTicketList($url,$params){
     	...
     	if((!empty($tId)) && ($tId != "1")){
			...
			//if(!empty($ipArray['submit'])){
				...
				if(!empty($ipArray['created_on'])){
					...
					$filtercinputArray['createdOnDate'] = date("Y-m-d",strtotime($ipArray['created_on']));
				}
     ...
     public function store(Request $request){
		...
		if((!empty($responseArray->status)) && ($responseArray->status == "200")){
			if(!empty($responseArray->msg)){
				if(!empty($userId)){
					return view('admin.ticket_form.tickets_list',['ticketdata' => $ticketListArray,'ticketcountdata'=> $ticketCountArray,'tab'=>$tabId,'error'=>'errorArray','department'=>$department,'category'=>$category,'type'=>$type,'priority'=>$priority,'source'=>$source,'priorityDisp'=>$priorityDispData,'post'=>$inputArray]);
				}
			}
		}
		elseif((!empty($responseArray->status)) && ($responseArray->status == "201")){
    ...
    public function ListUserPostFilter($url,$params){
		...
		*/
		...
		if(!empty($params['createdOnDate'])){
			$fields['createdOnDate'] = $params['createdOnDate'];
		}
