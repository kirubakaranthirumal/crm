	...
	*/
	public function index(Request $request){
		...
		if(!empty($cinputArray)){
			$appresults = SELF::ListAppPost("http://106.51.0.187:8090/cgwfollowon/activeapps",$cinputArray);
		}
...
<?php
	...
	public function load_group_user(Request $request){
		...
		$assignUserId = "";
		if(isset($inputArray['assignUserId']) && !empty($inputArray['assignUserId'])){
			$assignUserId = $inputArray['assignUserId'];
		}

		$assignerUserId = "";
		if(!empty($inputArray['assignUserId'])){
			$assignerUserId = $inputArray['assignUserId'];
		}

		if(!empty($userId)){
			return view('admin.ticket_form.select_group_user', ['userdata' => $groupUserListArray,'userid' => $assignUserId]);
		}
		else{
			return redirect('admin/login_user');
		}
	...
	public function createMailTicket(Request $request){
		
		$params = $departmentArray = $categoryArray = $typeArray = $mailNotificationArray = array();
		
		$department = $type = $category=array();
		$params = $request->all();

		if(!empty($params['emailNId'])){
			$results = SELF::loadEmailNotify("http://106.51.0.187:8090/cgwfollowon/findemailnotification", $params);
			$mailNotificationArray = json_decode($results);
		}
		
		//default value for create mail tickets and assign
		$mailNotificationArray->source = "2";
		$mailNotificationArray->priority = "3";
		$mailNotificationArray->status = "2";
		...
		$cinputArray = $appResponseArray = $appListArray = array();
		...
		return view('admin.ticket_form.create_mail_ticket',['app_data'=>$appListArray,'department'=>$department,'category'=>$category,'type'=>$type,'mailnotify'=>$mailNotificationArray]);
	}
	
	/* 
	* getting email notification details
	* param url and email id
	* using curl 
	*/
	public function loadEmailNotify($url, $params){

		if(!empty($params) && count($params) > 0){
			$json_string = json_encode($params);
		}
		$curl = curl_init($url);
		curl_setopt($curl, CURLOPT_HTTPHEADER, ['Content-Type: application/json','Accept-Language: en_US']);
		curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
		curl_setopt($curl, CURLOPT_POST, true);
		curl_setopt($curl, CURLOPT_POSTFIELDS, $json_string);
		$curl_response = curl_exec($curl);
