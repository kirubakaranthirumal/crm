...
class TicketReportGenController extends Controller{
	...
	*/
	public function index(Request $request){
		...
		$successArray = $errorArray = $ticketReportArray = $departmentArray = $categoryArray = $priorityArray = $sourceArray = $typeArray = $statusArray = array();
		...
		if(!empty($userId)){
			return view('admin.ticket_form.tickets_report',['app_data'=>$appListArray,'ticketdetails'=>$ticketReportArray,'department'=>$department,'category'=>$category,'type'=>$type,'priority'=>$priority,'source'=>$source,'status'=>$status]);
		}
		else{
	...
	*/
	...
	public function store(Request $request){
		...
		$formatted_end_date="";
		if(!empty($inputArray['start_date']) && !empty($inputArray['end_date'])){
			$formatted_start_date = date("Y/m/d",strtotime($inputArray['start_date']));
			$formatted_end_date = date("Y/m/d",strtotime($inputArray['end_date']));
			$where_str .= $join_str."create_on >= '".$formatted_start_date."'";
			$where_str .= $join_str."create_on <= '".$formatted_end_date."'";
		}
		elseif(!empty($inputArray['start_date'])){
			$formatted_start_date = date("Y/m/d",strtotime($inputArray['start_date']));
			$where_str .= $join_str."create_on > '".$formatted_start_date."'";
		}
		elseif(!empty($inputArray['end_date'])){
			$formatted_end_date = date("Y/m/d",strtotime($inputArray['end_date']));
			$where_str .= $join_str."create_on < '".$formatted_end_date."'";
		}
		...
		if(!empty($userId)){
			return view('admin.ticket_form.tickets_report',['app_data'=>$appListArray,'ticketReportArray'=>$ticketReportArray,'inputArray'=>$inputArray,'department'=>$department,'category'=>$category,'type'=>$type,'priority'=>$priority,'source'=>$source,'post'=>$inputArray,'status'=>$status]);
		}
		else{
			return redirect('admin/login_user');
		}
