...
class TeamListController extends Controller
{
    ...
    public function index(){
		...
		Session::forget('DeleteTeamSuccess');
    	Session::forget('DeleteTeamError');
        ...
        if(!empty($cinputArray)){
		 	$results = SELF::ListCountryPost("http://cgwrestapi-env.us-west-2.elasticbeanstalk.com/listcricketteam",$cinputArray);
		}
		...
		if(!empty($responseArray->cricketTeams)){
        	foreach($responseArray->cricketTeams as $responseVal){
				$teamListArray[] = $responseVal;
			}
		}
		
		//print"<pre>";
		//print_r($teamListArray);
		//exit;
		
		if((!empty($responseArray->status)) && ($responseArray->status == "200")){
            if(!empty($responseArray->msg)){
                if(!empty($userId)){
                	return view('admin.team.team_list', ['teamdata' => $teamListArray]);
                }
            }
        }
