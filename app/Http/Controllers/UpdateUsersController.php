		...
		if(!empty($inputArray['submit'])){
			...
			if(
				...
				if(!empty($cinputArray)){
					$user_results = SELF::EditUserPost("http://106.51.0.187:8090/cgwfollowon/findcrmuser",$cinputArray);
				}

				$responseArray = array();

				if(!empty($user_results)){
					$responseArray = json_decode($user_results);
				}
				...
				if(!empty($userId)){
					...
					if($usertype == 1){
						return view('admin.users.edit_user', ['userdata' => $userListArray, 'department'=>$departmentArray, 'id' => $userId]);
					}
					else{
