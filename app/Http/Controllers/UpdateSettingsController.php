...
class UpdateSettingsController extends Controller{
	...
	public function store(Request $request){
		...
		if(!empty($inputArray['settings_type'])){
			...
			elseif($inputArray['settings_type']== "6"){
				$statusId = $this->createStatus($inputArray);
			}
		}
		...
		if((!empty($deptId)) || (!empty($catId)) || (!empty($priorId)) || (!empty($sourceId)) || (!empty($typeId)) || (!empty($typeId))){
	...
	*/
	public function show($id, Request $request){
		...
		$deptUpdArrray = $catUpdArrray = $priorUpdArrray = $sourceUpdArrray = $typeUpdArrray = $statusUpdArray = array();
		...
		if(!empty($setId)){
			...
			elseif($setId=="6"){
				$statusValArray = $statusObj->retrieveByWhere($id);
			}
		}
		...
		if(!empty($params['submit'])){
			if(!empty($setTypeId)){
				...
				elseif($setTypeId=="6"){
					$statusUpdArray = $statusObj->updateById($id,$params);
				}
			}
		}
		...
		elseif(!empty($statusUpdArray)){
			session()->put('statusUpdSuc','Status has been Updated successfully');
		}
		...
		return view('admin.users.edit_settings',['department'=>$departmentArray,'category'=>$categoryArray,'priority'=>$priorityArray,'source'=>$sourceArray,'type'=>$typeArray,'status'=>$statusArray,'set_id'=>$setId,'deptval'=>$depValArray,'catval'=>$catValArray,'priorityval'=>$priorValArray,'sourceval'=>$sourceValArray,'typeval'=>$typeValArray,'statusval'=>$statusValArray]);
	}
