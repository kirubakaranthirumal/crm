		...
		if(
			...
			if(!empty($userId)){
				...
				if(!empty($ticketArray->ticketGroupId)){
					return view('admin.ticket_form.edit_tickets',['req_groupid'=>$ticketArray->ticketGroupId,'ticketdata'=>$ticketArray,'app_data'=>$appListArray]);
				}
				else{
					return view('admin.ticket_form.edit_tickets',['ticketdata'=>$ticketArray,'app_data'=>$appListArray]);
				}
			}
			else{
				return redirect('admin/login_user');
