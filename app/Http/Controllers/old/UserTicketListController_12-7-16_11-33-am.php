    ...
    public function index(Request $request){
		...
		if((!empty($responseArray->status)) && ($responseArray->status == "200")){
            if(!empty($responseArray->msg)){
                ...
                if(!empty($userId)){
                	return view('admin.ticket_form.user_tickets_list',['ticketdata' => $ticketListArray,'ticketcountdata'=> $ticketCountArray,'tab'=>$tabId, 'userType' => session()->get('userType')]);
                }
            }
        }
	    ...
	    if(!empty($userId)){
            return view('admin.ticket_form.user_tickets_list',['ticketdata' => $ticketListArray, 'userType' => session()->get('userType')]);
		}
		else{
