...
class UpdateEventController extends Controller
{
	 ...
	 public function show($id,Request $request){
		...
		if(!empty($inputArray['submit'])){
						...
						if(((!empty($postServiceArray)) && (!empty($responseArray->status="200"))))
				{
					...
					if(!empty($postServiceArray)){
						foreach ($postServiceArray as $value) {
							...
							$results = SELF::CreateAppEventPost("http://192.168.1.202:8085/demo/eventservice",$eventserviceinput);
							$checkedlist = SELF::GetEventListPost("http://192.168.1.202:8085/demo/takeservices",$getevenlist);		
			...
			if(!empty($cinputArray)){
				$results = SELF::EditEventPost("http://192.168.1.202:8085/demo/findevent",$cinputArray);
				$selectservice = SELF::EditEventPost("http://192.168.1.202:8085/demo/takeservices",$cinputArray);
			}
			$eventdata = $results;
