...
class UserLogStatusController extends Controller
{
    /**
...
/* 		Session::forget('DeleteUserSuccess');
        ...
        if(!empty($cinputArray)){
		 	$results = SELF::ListUserPost("http://106.51.0.187:8090/cgwfollowon/activeinactiveuser",$cinputArray);
		}
		...
		if(!empty($responseArray->activeUser)){
        	foreach($responseArray->activeUser as $responseVal){
		...
		if((!empty($responseArray->status)) && ($responseArray->status == "200")){
            if(!empty($responseArray->msg)){
                ...
                if(!empty($userId)){
                	return view('admin.users.user_log_status', ['activeuser' => $activeuserListArray,'inactiveuser' => $inactiveuserListArray]);
                }
            }
        }
	...
	public function getOnlineUser(){
		...
		$cinputArray = $responseArray = $userListArray = array();
		if(!empty($userId)){
