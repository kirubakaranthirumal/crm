...
class EventServiceListController extends Controller
{
	...
	public function show($id){
		...
		if(!empty($cinputArray)){
		 	$results = SELF::ListEventPost("http://106.51.0.187:8090/cgwfollowon/takeservices",$cinputArray);
		}
		...
		$cinputArray1 = $checkedservicelist = array();
		if(!empty($serviceIdArray)){
			foreach($serviceIdArray as $serviceId){
				$cinputArray1['serviceId'] = $serviceId->serviceId;
				$serviceresults = SELF::ListServicePost("http://106.51.0.187:8090/cgwfollowon/viewservice",$cinputArray1);
